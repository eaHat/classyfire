...
runPerm <- function (inputData, inputClass, bootNum = 100, ensNum = 100, permNum=100, parallel=TRUE, cpus=NULL, type = "SOCK", socketHosts = NULL, progressBar = TRUE) {
...
.snowRBFperm <- function(inputData, permMatr, bootNum, ensNum, permNum, parallel, cpus, type, socketHosts, progressBar) {
  tryCatch({
    ...
    for (k in 1:permNum) { 
      ...
      execTime = system.time(parComplexRes <- sfLapply(1:ensNum, .boxRadial))
